...
require_once ("administrator/models/Agendahotel.php");
	...
	public static function main () {
			...
			foreach ($_POST['nome'] as $dados) {
					...
					if(count($_POST['nome']) == $quantidade){						
						...
						$venda['id_local'] = $_POST['local'][$i_indexlocal];
						...
						if(!empty($_POST['cpf'])){
							$POST['id_cliente'] = str_replace("k","",$utimoid);
							$POST['cpf'] = $_POST['cpf'];
							$POST['telefone'] = $_POST['telefone'];
							$POST['status'] = 1;
							
							$objAgendaHotel->editarAgendahotel($POST,$_POST['id'][$i_indexlocal]);
						}
						$i_indexlocal++;
					}else{
		...
		$paymentRequest->setSender($strnome, $resCliente['email']);
